...
var rtp_vid_senders=[];
    ...
    function eventProcess(){
        ...
        $("#vidCamOnOff").on("click", async function(){
            if(video_st==video_states.Camera){
                await videoProcess(video_states.None);
                // Add the following line to set the local_div to a black screen when the camera is turned off
        ...
        $("#ScreenShareOnOff").on("click", async function(){
            if(video_st==video_states.ScreenShare){
                await videoProcess(video_states.None);
                // Add the following line to set the local_div to a black screen when the screen sharing is turned off
    ...
    function removeVideoStream(rtp_aud_senders){
        if(videoCamTrack){
            videoCamTrack.stop();
            videoCamTrack =null;
    ...
    async function videoProcess(newVideoState){
        if(newVideoState==video_states.None){
            $("#vidCamOnOff").html("<span class='material-icons' style='width: 100%;'>videocam_off</span>");
        ...
        if(newVideoState==video_states.Camera){
            $("#vidCamOnOff").html("<span class='material-icons' style='width: 100%;'>videocam_on</span>");
        }
        try{
            var vstream=null;
            if(newVideoState==video_states.Camera){
        ...
        video_st=newVideoState;
        if(newVideoState == video_states.Camera){
            $("#vidCamOnOff").html("<span class='material-icons' style='width:100%'>videocam</span>");
    ...
    async function SDPProcess(message,from_connid){
        ...
        if(message.answer){
            ...
            try{
                await peers_connection[from_connid].addIceCandidate(new RTCIceCandidate(message.icecandidate));
            }catch(e){
                console.log(e);
            }
        }
    ...
    return{
        ...
        processClientFunc:async function(data,from_connid){
         ...
         closeConnectionCall:async function(data,connid){
...
$(document).on("click",".top-left-participant-wrap",function(){
 ...
 $(document).on("click",".end-call-wrap",function(){
    $(".top-box-show").css({
        "display":"block"
    }).html(' <div class="top-box align-vertical-middle profile-dialogue-show justify-content-center" style="text-align:center;color:white;font-size:15px"> <h3 class="mt-2">Leave Meeting</h3><hr> <div class="call-leave-cancel-action d-flex justify-content-center align-items-center w-100"> <a href="/action.html"><button class="call-leave-action btn btn-danger mr-5">Leave</button></a> <button class="call-cancel-action btn btn-secondary">Cancel</button> </div> </div>');
